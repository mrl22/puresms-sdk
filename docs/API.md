...
## `formatNumber()` and `toE164()`

```php
PureSms::formatNumber(string|int $number, string|int $countryCode = '44'): string
PureSms::toE164(string|int $number, string|int $countryCode = '44'): string
```

`formatNumber()` formats a phone number without making an API request; `toE164()` is its equivalent alias. The default country calling code is `44`, which makes these UK input forms equivalent:

```php
PureSms::formatNumber('07590123456');   // +447590123456
PureSms::formatNumber('447590123456');  // +447590123456
PureSms::formatNumber('4407590123456'); // +447590123456
```
